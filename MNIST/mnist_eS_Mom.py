...
momentum = 0.9
velocity = [tf.zeros_like(var) for var in model.trainable_variables]
...
for epoch in range(epochs):
    ...
    for batch_idx, i in enumerate(range(0, len(x_train), batch_size)):
        ...
        aggregator = [a - g for a, g in zip(aggregator, grad_tab[batch_idx])]
        # Save gradient for this batch
        grad_tab[batch_idx] = [tf.identity(grad) for grad in gradients]
        # update aggregator
        aggregator = [a + g for a, g in zip(aggregator, grad_tab[batch_idx])]
        ## Momentum equivalent if uncomment
        #aggregator = [tf.identity(grad) for grad in gradients]
        # Momentum update: v = momentum * v - grad
        velocity = [momentum * v + g for v, g in zip(velocity, aggregator)]
        # Apply update: w += lr * v
        optimizer.apply_gradients(zip(velocity, model.trainable_variables))
...
metrics_df.to_csv("SpethMom_batch_metrics.csv", index_label="epochs")
...
fig.tight_layout()
fig.savefig("SpethMom_loss_accuracy_plot.png", dpi=300)
plt.show()
