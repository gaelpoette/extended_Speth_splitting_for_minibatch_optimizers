...
for epoch in range(epochs):
    ...
    for batch_idx, i in enumerate(range(0, len(x_train), batch_size)):
        ...
        # Store gradients in aggregator specific to this batch
        grad_tab[batch_idx] = [tf.identity(grad) for grad in gradients]
        # update aggregator
        aggregator = [a + g for a, g in zip(aggregator, grad_tab[batch_idx])]
        # SGD equivalent if uncomment
...
metrics_df.to_csv("SpethGD_batch_metrics.csv", index_label="epochs")
...
fig.tight_layout()
fig.savefig("SpethGD_loss_accuracy_plot.png", dpi=300)
plt.show()
